...
all_message_for_each_language = list(
    map((
        lambda language : TEXT.get_text(language)
        ), all_language))


def get_selected_language(text):
    dict_with_all_messages = TEXT.all_text[TEXT.language]
...
def selecting_the_language(update, context):
    try: 
        ...
        TEXT.change_language(selected_language)
        text = get_text("selected_language")
        update.message.reply_text(text, reply_markup=ReplyKeyboardRemove())
    except:
        text = get_text("language_dont_exist")
        update.message.reply_text(text, reply_markup=ReplyKeyboardRemove())
    finally:
        return END
